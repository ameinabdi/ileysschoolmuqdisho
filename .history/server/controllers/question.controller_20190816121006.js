...
export function findAllQuestion(req, res){
    ...
    var _counter = 0;
    var _values = [];
    connection.query("SELECT * FROM classes INNER JOIN subjects ON classes.classId = subjects.classId INNERT JOIN question ON classes.classId = question.classId AND subjects.subjectId = question.SubjectId ").then((result)=>{
        res.json(result)
    }).catch((err)=>{
        res.json(err)
    })
