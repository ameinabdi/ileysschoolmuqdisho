...
export function findAllQuestion(req, res){
    ...
    connection.query("SELECT * FROM classes ").then((result)=>{
        ...
        //     // res.json(result[i].className)
        //     console.log(result[i].classId)
        //      connections.query("SELECT * FROM subjects WHERE classId= ?", result[i].classId).then((Subject)=>{
        //          const Data = {
        ...
        result.map((items, index)=>{
            console.log(items.classId)
            connection.query("SELECT * FROM subjects WHERE classId = ?", items.classId, (error, result)=>{
                if(error){
                    res.json(error)
                } else{
                  result.map((subjectitems, index)=>{
                    _values.push(subjectitems)
                  })
                }
            })
            _values2 = ({classId:items.classId, className:items.className, subjects:[_values]})
            res.json(_values)
