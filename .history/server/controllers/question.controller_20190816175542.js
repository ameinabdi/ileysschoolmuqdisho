    ...
    connection.query("SELECT * FROM classes ").then((result)=>{
        result.foreach((items, key)=>{
            connection.query("SELECT * FROM subjects where classId= ?", items.classId).then((subjects)=>{
                subjects.foreach((subjectitems, key)=>{
                    connection.query("SELECT * FROM question WHERE subjectId = ?", subjectitems.subjectId, (result, err)=>{
                        if(err){
                            res.json(err)
                        }else{
                            console.log('saasas',items)
                            const data = { 
                            "classId":items.classId,
                            "className":items.className,
                            "subject":[{
                                "subjectName" :subjectitems.subjectName,
                                "subjectId": subjectitems.subjectId,
                                "quiz": result
                            }]}
                            if(_counter++ == subjectitems.length) 
                            res.json(data)
                        }
                    })
