...
export function findAllQuestion(req, res){
    ...
    var _values = [];
    var _values2 = [];
    connection.query("SELECT classes.*, subjects.*, question.* FROM classes  INNER JOIN subjects  ON classes.classId = subjects.classId LEFT JOIN question  ON subjects.subjectId = question.SubjectId").then((result)=>{
        var scheme = {
