...
export function addNewSubject(req, res) {
    ...
    connection.query(`select * from classes where classId =  ${classId}`)
        .then(Subjects => {
            connection.query("INSERT INTO subjects(`subjectName`,`classId`,`className`, `created_at`,`updated_at`)VALUES ('"+ subjectName+"','"+ classId+"','" +Subjects[0].className+ "', '" +created_at+ "', '" +updated_at+ "')")
            .then((result)=>{
                res.json({
                    success: true,
                    data:result
