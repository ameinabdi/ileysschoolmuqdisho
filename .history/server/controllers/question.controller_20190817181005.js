...
export function findAllQuestion(req, res){
    ...
    var _values = [];
    var _values2 = [];
    connection.query("SELECT * FROM classes INNER JOIN subjects ON classes.classId = subjects.subjectId INNER JOIN question ON subjects.subjectId = question.SubjectId").then((result)=>{
        var scheme = {
            "$group[questions](classId)": {
              "id": "classId",
              "class": "className",
              "$group[sections](subjectId)": {
                "sectionid": "subjectId",
