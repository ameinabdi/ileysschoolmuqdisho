...
export function findAllQuestion(req, res){
    ...
    var _values = [];
    var _values2 = [];
    connection.query("SELECT classes.className, classes.classId, subjects.subjectId,subjects.subjectName, subjects.classId, question.Title, question.question, question.marks, question.Qid FROM classes  INNER JOIN subjects  ON classes.classId = subjects.classId LEFT JOIN question  ON subjects.subjectId = question.SubjectId").then((result)=>{
        var scheme = {
...
export function findAll(req, res){
  ...
  var _values = [];
  var _values2 = [];
  connection.query("SELECT  * from question").then((result)=>{
     res.json({
         success: true,
         data:result
     })
