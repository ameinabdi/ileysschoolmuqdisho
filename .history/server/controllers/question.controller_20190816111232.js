...
export function findAllQuestion(req, res){
    ...
    var _counter = 0;
    var _values = [];
    connection.query("SELECT JSON_OBJECT('classId' classes.classId, 'className' classes.className, 'subjects' JSON_ARRAYAGG(JSON_OBJECT('subjectName', subjects.SubjectName)) from classes INNER JOIN subject ON classes.classId = subject.classId GROUP BY classes.classId )").then((result)=>{
        res.json(result)
    }).catch((err)=>{
        res.json(err)
    })
