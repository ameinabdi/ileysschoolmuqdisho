...
export function findAllQuestion(req, res){
    ...
    var _counter = 0;
    var _values = [];
    connection.query("SELECT JSON_OBJECT('ID',classId, 'Class',ClassName, JSON_OBJECT('ID',SubjectId,'Name', subjectName,JSON_OBJECT('Title',title, 'Marks',marks,'Question',question))) AS result FROM question GROUP BY Qid").then((result)=>{
        res.json(result)
    }).catch((err)=>{
        res.json(err)
    })
