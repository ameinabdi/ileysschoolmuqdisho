...
export function findAllQuestion(req, res){
    var _counter = 0;
    var _values = [];
    connection.query("SELECT JSON_OBJECT('className' classes.className, 'subjects', JSON_OBJECT(subjectName 'subjects.SubjectName', 'question', JSON_ARRAYAGG(JSON_OBJECT('id',question.Qid, 'title',question.title, 'marks',question.marks,'question',question.question)))) FROM classes INNER JOIN subjects ON classes.classId = subjects.classId  INNER JOIN question ON class.classId = question.classId").then((result)=>{
        res.json(result)
    }).catch((err)=>{
        res.json(err)
    })
