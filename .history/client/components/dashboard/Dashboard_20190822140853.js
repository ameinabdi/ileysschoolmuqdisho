          ...
          children: [
              new Paragraph([
                    ...
                    .map((item) => {
                        const arr: item[] = [];
                        arr.push(
                            this.createInstitutionHeader(item.question, `${item.marks} - ${item.marks}`),
                        );
                        arr.push(this.createRoleText(`${item.question} - ${item.question}`));

                        const bulletPoints = this.splitParagraphIntoBullets(item.question);
