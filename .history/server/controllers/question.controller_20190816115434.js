...
export function findAllQuestion(req, res){
    ...
    var _counter = 0;
    var _values = [];
    connection.query("SELECT JSON_ARRAY(classId, ClassName, JSON_ARRAY(JSON_OBJECT(subjecId,subjectName,JSON_ARRAY(JSON_OBJECT(title, marks,question)))))  FROM question GROUP BY Qid").then((result)=>{
        res.json(result)
    }).catch((err)=>{
        res.json(err)
    })
