...
export function findAllQuestion(req, res){
    ...
    var _values = [];
    var _values2 = [];
    connection.query("SELECT * FROM classes INNER JOIN subjects ON classes.classId = subjects.subjectId INNER JOIN question ON subjects.subjectId = question.SubjectId AND subjects.classId = question.classId").then((result)=>{
        var scheme = {
            "$group[questions](classId)": {
              "id": "classId",
              "name": "className",
              "$group[projects](subjectId)": {
                "id": "subjectId",
                "name": "SubjectName"
              }
            }
