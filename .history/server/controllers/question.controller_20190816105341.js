...
export function findAllQuestion(req, res){
    var _counter = 0;
    var _values = [];
    connection.query("SELECT className, JSON_OBJECTAGG(subjectName JSON_OBJECTAGG(question) as questions from question group by subjectName) AS attributes  FROM question GROUP BY classId ").then((result)=>{
        res.json(result)
    }).catch((err)=>{
        res.json(err)
    })
