...
export function findAllQuestion(req, res){
    ...
    var _counter = 0;
    var _values = [];
    connection.query("SELECT JSON_ARRAYAGG(classId, ClassName, JSON_ARRAY(SubjectId,subjectName,JSON_ARRAY(title, marks,question))) AS result FROM question GROUP BY Qid").then((result)=>{
        res.json(result)
    }).catch((err)=>{
        res.json(err)
    })
